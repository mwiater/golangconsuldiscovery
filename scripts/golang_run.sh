...
echo -e "${CYANBOLD}Building Go app:${RESET} ${GREENBOLD}go build -o bin/golangconsuldiscovery .${RESET}"
go build -o bin/golangconsuldiscovery .
status=$?
if test $status -ne 0
then
...
echo -e "${CYANBOLD}Running Go app:${RESET} ${GREENBOLD}./bin/golangconsuldiscovery${RESET}"
./bin/golangconsuldiscovery